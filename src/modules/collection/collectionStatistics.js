...
const collectionStatistics = async () => {
  ...
  const nowDate = Math.max(
    ...(Object.keys(prevErrorTournaments).length > 0
      ? Object.keys(prevErrorTournaments)
      : [0])
  );
  const nowError = {};
      ...
      Object.keys(stateAlias).map(async (alias) => {
        ...
        if (!tournaments.length) {
          ...
          tournaments.forEach((ft) => {
            ...
            const isStartDate = Number(
              t["@date"] ?? t["@scheduledStartDate"] ?? 0
            );
            const time = getTimeByMS(Number(`${isStartDate - d}000`));

            const abilityBid =
              ability2?.[network]?.[level]?.[currency]?.[realBid]?.[status] ??
              0;

            const rulesAbility2 = rules[network]?.[time]?.[level]?.[currency]?.[
              realBid
            ]?.[status]?.[t["@name"]]
              ? rules[network]?.[time]?.[level]?.[currency]?.[realBid]?.[
                  status
                ]?.[t["@name"]]
              : rules[network]?.["all"]?.[level]?.[currency]?.[realBid]?.[
                  status
                ]?.["all"]
              ? rules[network]?.["all"]?.[level]?.[currency]?.[realBid]?.[
                  status
            ...
            const info =
              ability1?.[network]?.[time]?.[bid]?.[name]?.["@avability"];
